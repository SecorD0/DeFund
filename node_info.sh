...
while test $# -gt 0; do
	...
	-h|--help)
		...
		echo
		echo -e "${C_LGn}Functionality${RES}: the script shows information about a DeFund node"
		echo
		...
		echo -e "https://github.com/SecorD0/DeFund/blob/main/node_info.sh - script URL"
...
# Config
daemon="`which defundd`"
token_name="fetf"
node_dir="$HOME/.defund/"
wallet_name="$defund_wallet_name"
wallet_address="$defund_wallet_address"
wallet_address_variable="defund_wallet_address"
validator_address="$defund_validator_address"
validator_address_variable="defund_validator_address"
...
main() {
	...
	# Send Pull request with new texts to add a language - https://github.com/SecorD0/DeFund/blob/main/node_info.sh
	#elif [ "$language" = ".." ]; then
	else
